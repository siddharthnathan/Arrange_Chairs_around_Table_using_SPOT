...
def main():
    ...
    try:
        ...
        while True:
            ...
            _ = pose_estimation.estimate_poses_of_aruco_tags(camera_1_frame, aruco_type, camera_calibration_params['Camera_1']) 
            _ = pose_estimation.estimate_poses_of_aruco_tags(camera_2_frame, aruco_type, camera_calibration_params['Camera_2'])

            # Save Initial images when 1 is Pressed
            if cv2.waitKey(1) == ord('1'):
                cv2.imwrite('initial_image_1.jpg', camera_1_frame)
                cv2.imwrite('initial_image_2.jpg', camera_2_frame)
            
            # Save Final images when 2 is Pressed
            elif cv2.waitKey(1) == ord('2'):
                cv2.imwrite('final_image_1.jpg', camera_1_frame)
                cv2.imwrite('final_image_2.jpg', camera_2_frame)
