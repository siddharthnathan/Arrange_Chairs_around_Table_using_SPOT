...
class Coordinate_Frame_Transformation:
    ...
    def __init__(self, name, reference_frame, translation_vector, rotation_angles):
        ...
        # Else when Reference frame is given
            ...
            self.rotation_matrix = utils.round_matrix_list(reference_frame.rotation_matrix @ self.rotation_matrix, 3)
...
# Define a Function to Compute the Gripper pose to Grasp chair
def compute_gripper_pose_to_grasp_chair(aruco_tags_data_wrt_world_frame):

    # For every AruCo tag detected in world frame
    for aruco_tag_data_wrt_world_frame in aruco_tags_data_wrt_world_frame:

        # If Aruco tag is on Chair
        if "Chair" in aruco_tag_data_wrt_world_frame.name:

            # Get the Pose where Chair should be grasped wrt World frame
            chair_grasp_pose = Coordinate_Frame_Transformation(
                                                                    name = 'Chair_Grasp_Pose',
                                                                    reference_frame = aruco_tag_data_wrt_world_frame,
                                                                    translation_vector = [-0.024, -0.383, 0.178],
                                                                    rotation_angles = [31.515, -25.615, 0]
                                                              )
        
        # If AruCo tag is on SPOT
        elif "SPOT" in aruco_tag_data_wrt_world_frame.name:

            # Get the Pose of SPOT gripper wrt World frame
            spot_gripper_pose = Coordinate_Frame_Transformation(
                                                                    name = 'SPOT_Gripper_Pose',
                                                                    reference_frame = aruco_tag_data_wrt_world_frame,
                                                                    translation_vector = [0.810, 0.274, 0.182],
                                                                    rotation_angles = [57.597, 82.166, -4]
                                                               )
    chair_grasp_pose.display()
    spot_gripper_pose.display()
    
    # Compute and return the Chair grasp Pose wrt SPOT gripper pose in World frame
    T_spot_gripper_wrt_world_frame = utils.round_matrix_list(np.linalg.inv(spot_gripper_pose.transformation_matrix) @ chair_grasp_pose.transformation_matrix, 3)

    # Extract Translation and Rotation components and Return it
    spot_gripper_pose_wrt_world_frame = {}
    spot_gripper_pose_wrt_world_frame['Translation'] = T_spot_gripper_wrt_world_frame[:3, 3]
    spot_gripper_pose_wrt_world_frame['Rotation'] = utils.round_float_list(R.from_matrix(T_spot_gripper_wrt_world_frame[:3, :3]).as_euler('xyz', degrees = True), 3)
    return spot_gripper_pose_wrt_world_frame